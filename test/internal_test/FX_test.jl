...
function plot_graph(result_path)
	...
	df_numberOfYears = np.unique(df.index.year)
	for (_, year) in enumerate(df_numberOfYears)
		data = get(df.loc, (string(year)))
		MRP_init = MRP.Market_regime_plot(data, data_freq="m")
